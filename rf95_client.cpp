...
#define RF_GATEWAY_ID 20
#define RF_NODE_ID    36
  ...
  if (!rf95.init()) {
    ...
    while (!force_exit) {
      ...
      if ( millis() - last_millis > 5000 ) {
        ...
        FILE *fp;
        char readBuff[256],readBuff2[256];
        fp = popen("mosquitto_sub -h serveur.iot.com -p 1883 -t \"/esp8266\" -u esp -P tmctmctmc -C 1","r");
        if (fp == NULL) {
          printf("Failed to run command\n" );
          exit(1);
        }
        fgets(readBuff,256,fp);
        length = strlen(readBuff);
        buffer = (char*)malloc(sizeof(char)*length);
        readBuff[strcspn(readBuff, "\n")] = 0;
        strcpy(buffer,readBuff);
        printf("Input : %s\n",buffer);
        char * start = "echo ";
        char * end = " | openssl enc -aes-256-cbc -pbkdf2 -salt -a -k test";
        char *cmd = (char*) malloc( (sizeof(char)* (strlen(start)+strlen(end)+length)));
        strcat(cmd,start);
        strcat(cmd,buffer);
        strcat(cmd,end);
        //printf("%s\n",cmd);
        ...
        fgets(readBuff2,256,fp);
        readBuff2[strcspn(readBuff2, "\n")] = 0;
        length = strlen(readBuff2);
        buffer = (char*)malloc(sizeof(char)*length);
        strcpy(buffer,readBuff2);
        printf("Cipher : %s\n",buffer);

        //printf("%s\n",buffer);
        printf("Sending %d bytes to node #%d => ", length, RF_GATEWAY_ID );
        pclose(fp);
        printbuffer((uint8_t*)buffer, length);
        rf95.send((uint8_t*)buffer, length);
        rf95.waitPacketSent();
        printf("\n");
        free(cmd);
        free(buffer);
